...
int main(int _argc, char **_argv) {
    ...
    while (fgets(buffer, sizeof(buffer), stdin)) {
        ...
        if (strcmp(command, "create-student") == 0) {
            ...
            courses[course_size++] = create_cours=e(code, name, credit, grade);
            ...
            char *code = strtok(NULL, "#");
            struct course_t c = find_course_by_code(courses, course_size, code);
            if (strlen(c.code) > 0) print_course(c);
        }
    }
